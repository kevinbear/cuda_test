...
//======================//
//===========kernel function=========//
__global__ void yuv2rgb1(unsigned char *d_y ,unsigned char *d_u,unsigned char *d_v,unsigned char *d_r,unsigned char *d_g,unsigned char *d_b,unsigned char *ty,unsigned char *tu,unsigned char *tv)
{
	...
	for(int i=0;i<cui;i++)
	{
		...
		ty[ind+(i*cysize)]=ydata[ind];
		tu[ind_uv+(i*cuvsize)]=udata[ind_uv];
		tv[ind_uv+(i*cuvsize)]=vdata[ind_uv];
		__syncthreads();
		//====================================================
		//printf("i=%d ind=%d ind_uv=%d \n",i,ind+(i*cysize),ind_uv+(i*cuvsize));			
...
FILE *fpw=fopen("debug.yuv","wb");
/*FILE *fp=fopen("index.xls","wb");
FILE *fp1=fopen("indexuv.xls","wb");*/
unsigned char *in_y=NULL,*in_u=NULL,*in_v=NULL,*out_r=NULL,*out_g=NULL,*out_b=NULL,*out_y=NULL,*out_u=NULL,*out_v=NULL;
unsigned char *d_y,*d_r,*d_u,*d_v,*d_g,*d_b,*ty,*tu,*tv;
//int *dex,*uvdex;
...
int main()
{
	...
	out_y=(unsigned char*)malloc(sizeof(unsigned char)*ysize);
	out_u=(unsigned char*)malloc(sizeof(unsigned char)*ysize/4);
	out_v=(unsigned char*)malloc(sizeof(unsigned char)*ysize/4);
	//for debug (index)
	...
	cudaMalloc((void **)&ty,sizeof(unsigned char)*ysize);
	cudaMalloc((void **)&tu,sizeof(unsigned char)*ysize/4);
	cudaMalloc((void **)&tv,sizeof(unsigned char)*ysize/4);
	...
	cudaMemset(ty,0,sizeof(unsigned char)*ysize);
	cudaMemset(tu,0,sizeof(unsigned char)*ysize/4);
	cudaMemset(tv,0,sizeof(unsigned char)*ysize/4);
	...
	while(1)
	{
		if(fread(in_y,1,ysize,fp)!=NULL)
		{
			...
			yuv2rgb1 <<<blks,thnum>>>(d_y,d_u,d_v,d_r,d_g,d_b,ty,tu,tv); //yuv2rgb kernel function
			...
			cudaMemcpy(out_y,ty,sizeof(unsigned char)*ysize,cudaMemcpyDeviceToHost);
			cudaMemcpy(out_u,tu,sizeof(unsigned char)*ysize/4,cudaMemcpyDeviceToHost);
			cudaMemcpy(out_v,tv,sizeof(unsigned char)*ysize/4,cudaMemcpyDeviceToHost);	
			//===============================================//
			fwrite(out_y,1,ysize,fpw);
			fwrite(out_u,1,ysize/4,fpw);
			fwrite(out_v,1,ysize/4,fpw);
